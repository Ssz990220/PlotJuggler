...
The icon-generation scripts have been removed from this MR. To
regenerate the icon set (e.g. after adding a new entry to the
...
- `pj_widgets/SvgUtil.h::RecolorSvgInk()` — swaps `#3D3D3D` ↔
...
- `pj_app/src/Style.h` — `QProxyStyle` over Fusion that suppresses
  the platform's auto-injected dialog-button icons and mnemonic
  underlines.
