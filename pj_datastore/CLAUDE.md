...
## Layout
- `include/pj_datastore/` — public headers (engine/writer/reader/query/chunk, `object_store`, `derived_engine` + `builtin_transforms`, `plugin_data_host`, `colormap_registry`, `arrow_import`, low-level buffer/column_buffer/encoding/topic_storage/type_registry, plus the data-processor substrate `data_processor`/`sample`/`processor_siso_adapter`/`processor_detail` — namespace `PJ::proc`).
