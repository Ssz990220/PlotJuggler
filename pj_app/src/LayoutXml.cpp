...
void normalizePlotRangeBasis(QDomDocument& doc) {
  ...
  for (int index = 0; index < plots.size(); ++index) {
    ...
    // No numeric values change. XY and Snapshot plots stored a raw data value on X
    // (the snapshot writer has always emitted raw left/right, never the display
    // offset); time-series plots have always stored ABSOLUTE seconds (the v3 writer
    // added the display offset), so the pre-marker form maps directly onto today's
    // two bases. Treat an unmarked Snapshot range as value-basis so its X frame is
    // not shifted by the ~1.6e9 s epoch offset on load/undo/redo.
    const QString mode = plot.attribute(u"mode"_s);
    const bool is_value_basis = (mode == u"XYPlot"_s) || (mode == u"Snapshot"_s);
    range.setAttribute(plot_xml::kXBasisAttribute,
                       is_value_basis ? plot_xml::kXBasisValue : plot_xml::kXBasisAbsolute);
  }
}
