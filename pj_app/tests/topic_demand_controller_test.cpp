...
QString addScalarTopic(PJ::AppSession& app_session, DatasetId dataset_id, std::string_view topic_name) {
  ...
  const auto key = PJ::resolveSeriesPath(
      ...
      PJ::layout_xml::SeriesPath{
          QString::fromUtf8(topic_name.data(), static_cast<int>(topic_name.size())), QStringLiteral("value")});
...
class StubSceneLayer : public PJ::ISceneLayer {
 public:
  ...
  [[nodiscard]] PJ::SceneLayerInfo info() const override {
    return PJ::SceneLayerInfo{topic_id_, object_type_, name_, QStringLiteral("stub"), true};
  }
...
TEST_F(TopicDemandControllerTest, PlotCurveAddRemoveTracksDemand) {
  ...
  EXPECT_FALSE(referencesTopic(app_session_->topicDemandTracker(), dataset_id_, QStringLiteral("/speed")));

  ASSERT_NE(plot.addCurve(key), nullptr);
  EXPECT_TRUE(referencesTopic(app_session_->topicDemandTracker(), dataset_id_, QStringLiteral("/speed")));
...
TEST_F(TopicDemandControllerTest, XyCurveReferencesBothSources) {
  ...
  controller_->registerPlot(&plot);
  plot.setModeXY(true);
  ASSERT_NE(plot.addCurveXY(x_key, y_key, QStringLiteral("xy")), nullptr);

  auto& tracker = app_session_->topicDemandTracker();
  EXPECT_TRUE(referencesTopic(tracker, dataset_id_, QStringLiteral("/pose_x")));
  EXPECT_TRUE(referencesTopic(tracker, dataset_id_, QStringLiteral("/pose_y")));

  plot.removeAllCurves();
  EXPECT_FALSE(referencesTopic(tracker, dataset_id_, QStringLiteral("/pose_x")));
  EXPECT_FALSE(referencesTopic(tracker, dataset_id_, QStringLiteral("/pose_y")));
}
...
TEST_F(TopicDemandControllerTest, DerivedFilterOutputReferencesSourceNotOutput) {
  ...
  const auto output_key = PJ::resolveSeriesPath(
      app_session_->catalogModel(),
      PJ::layout_xml::SeriesPath{QStringLiteral("speed[Absolute]"), QStringLiteral("value")});
  ...
  EXPECT_TRUE(referencesTopic(tracker, dataset_id_, QStringLiteral("/speed")));
  EXPECT_FALSE(referencesTopic(tracker, dataset_id_, QStringLiteral("speed[Absolute]")));
}
...
TEST_F(TopicDemandControllerTest, PlaceholderPlotDropStagesPendingBindThatCompletesOnRealTopic) {
  ...
  catalog.setAdvertisedTopics(
      dataset_id_, {PJ::AdvertisedTopic{QStringLiteral("/speed"), PJ::sdk::BuiltinObjectType::kNone}});

  PJ::PlotWidget plot(&app_session_->sessionManager(), &app_session_->catalogModel());
  controller_->registerPlot(&plot);

  controller_->handlePlaceholderPlotDrop(&plot, dataset_id_, QStringLiteral("/speed"));
  auto& tracker = app_session_->topicDemandTracker();
  EXPECT_TRUE(referencesTopic(tracker, dataset_id_, QStringLiteral("/speed")));
  ...
  const QString key = addRawScalarTopic(*app_session_, dataset_id_, QStringLiteral("/speed"));
  ASSERT_FALSE(key.isEmpty());
  EXPECT_EQ(binder_->flush(QSet<QString>{QStringLiteral("/speed")}), 1);
...
TEST_F(TopicDemandControllerTest, PendingBindAttachesDemandWhenTopicIsAdvertisedLater) {
  ...
  PJ::PlotWidget plot(&app_session_->sessionManager(), &app_session_->catalogModel());
  binder_->addPendingCurve(&plot, PJ::layout_xml::SeriesPath{QStringLiteral("/lidar"), QStringLiteral("range")});
  EXPECT_FALSE(referencesTopic(tracker, dataset_id_, QStringLiteral("/lidar")));
  ...
  app_session_->catalogModel().setAdvertisedTopics(
      dataset_id_, {PJ::AdvertisedTopic{QStringLiteral("/lidar"), PJ::sdk::BuiltinObjectType::kNone}});
  EXPECT_EQ(binder_->flush(QSet<QString>{QStringLiteral("/lidar")}), 0);  // cannot bind yet — no data
  EXPECT_TRUE(referencesTopic(tracker, dataset_id_, QStringLiteral("/lidar")));
}
...
TEST_F(TopicDemandControllerTest, SceneDockLayerAddRemoveTracksDemand) {
  addObjectTopic(*app_session_, dataset_id_, QStringLiteral("/points"));

  StubSceneDockWidget dock;
  controller_->registerSceneDock(&dock);

  const ObjectTopicId topic_id = findObjectTopicId(app_session_->catalogModel(), QStringLiteral("/points"));
  ASSERT_NE(topic_id.id, 0U);

  ASSERT_TRUE(dock.addTopic(topic_id, PJ::sdk::BuiltinObjectType::kPointCloud, QStringLiteral("/points")));
  auto& tracker = app_session_->topicDemandTracker();
  EXPECT_TRUE(referencesTopic(tracker, dataset_id_, QStringLiteral("/points")));

  dock.removeTopic(topic_id);
  EXPECT_FALSE(referencesTopic(tracker, dataset_id_, QStringLiteral("/points")));
}
...
TEST_F(TopicDemandControllerTest, SceneDockPlaceholderDropCompletesOnRealTopicWithoutDoubleCounting) {
  ...
  app_session_->catalogModel().setAdvertisedTopics(
      dataset_id_, {PJ::AdvertisedTopic{QStringLiteral("/points"), PJ::sdk::BuiltinObjectType::kPointCloud}});

  auto dock = std::make_unique<StubSceneDockWidget>();
  controller_->registerSceneDock(dock.get());

  controller_->handleSceneDockPlaceholderDrop(dock.get(), dataset_id_, QStringLiteral("/points"));
  auto& tracker = app_session_->topicDemandTracker();
  EXPECT_TRUE(referencesTopic(tracker, dataset_id_, QStringLiteral("/points")));
  ...
  addObjectTopic(*app_session_, dataset_id_, QStringLiteral("/points"));
  EXPECT_EQ(binder_->flush(QSet<QString>{QStringLiteral("/points")}), 1);
  EXPECT_TRUE(referencesTopic(tracker, dataset_id_, QStringLiteral("/points")));
  const ObjectTopicId topic_id = findObjectTopicId(app_session_->catalogModel(), QStringLiteral("/points"));
  ASSERT_NE(topic_id.id, 0U);
...
TEST_F(TopicDemandControllerTest, SceneDropSurvivesStreamReconnectMintingFreshDatasetId) {
  ...
  catalog.setAdvertisedTopics(
      dataset_b, {PJ::AdvertisedTopic{QStringLiteral("/points"), PJ::sdk::BuiltinObjectType::kPointCloud}});
  static_cast<void>(binder_->flush({}));
  EXPECT_TRUE(referencesTopic(tracker, dataset_b, QStringLiteral("/points")))
  ...
  addObjectTopic(*app_session_, dataset_b, QStringLiteral("/points"));
  static_cast<void>(binder_->flush(QSet<QString>{QStringLiteral("/points")}));
  const ObjectTopicId topic_id = findObjectTopicId(app_session_->catalogModel(), QStringLiteral("/points"));
  ASSERT_NE(topic_id.id, 0U);
  ...
  dock.reset();
  EXPECT_FALSE(referencesTopic(tracker, dataset_id_, QStringLiteral("/points")));
  EXPECT_FALSE(referencesTopic(tracker, dataset_b, QStringLiteral("/points")));
}
...
TEST_F(TopicDemandControllerTest, PlotDestructionReleasesUnresolvedPlaceholderDropReferenceEagerly) {
  ...
  auto plot = std::make_unique<PJ::PlotWidget>(&app_session_->sessionManager(), &app_session_->catalogModel());
  controller_->registerPlot(plot.get());
  controller_->handlePlaceholderPlotDrop(plot.get(), dataset_id_, QStringLiteral("/speed"));

  auto& tracker = app_session_->topicDemandTracker();
  EXPECT_TRUE(referencesTopic(tracker, dataset_id_, QStringLiteral("/speed")));

  plot.reset();  // no flush after this — release must be eager
  EXPECT_FALSE(referencesTopic(tracker, dataset_id_, QStringLiteral("/speed")));
}
...
TEST_F(TopicDemandControllerTest, AllPendingSceneDropsForOneTopicComplete) {
  ...
  controller_->handleSceneDockPlaceholderDrop(dock_a.get(), dataset_id_, QStringLiteral("/points"));
  controller_->handleSceneDockPlaceholderDrop(dock_b.get(), dataset_id_, QStringLiteral("/points"));

  addObjectTopic(*app_session_, dataset_id_, QStringLiteral("/points"));
  // BOTH pending drops complete on one flush pass (not just the first match).
  EXPECT_EQ(binder_->flush(QSet<QString>{QStringLiteral("/points")}), 2);
  const ObjectTopicId topic_id = findObjectTopicId(app_session_->catalogModel(), QStringLiteral("/points"));
  ASSERT_NE(topic_id.id, 0U);
  ...
  dock_a.reset();
  EXPECT_TRUE(referencesTopic(tracker, dataset_id_, QStringLiteral("/points")))
      << "dock B's layer still references the topic";
  dock_b.reset();
  EXPECT_FALSE(referencesTopic(tracker, dataset_id_, QStringLiteral("/points")))
...
TEST_F(TopicDemandControllerTest, CensusPreviewsScalarPlaceholderAndReleasesOnPromotion) {
  ...
  catalog.setAdvertisedTopics(
      dataset_id_, {PJ::AdvertisedTopic{QStringLiteral("/imu"), PJ::sdk::BuiltinObjectType::kNone}});
  // The census auto-previews the scalar placeholder: a demand reference is held.
  EXPECT_TRUE(referencesTopic(tracker, dataset_id_, QStringLiteral("/imu")));
  ...
  ASSERT_FALSE(key.isEmpty());
  EXPECT_FALSE(referencesTopic(tracker, dataset_id_, QStringLiteral("/imu")));
...
TEST_F(TopicDemandControllerTest, PreviewSampleIsDisownedWhenTopicGainsRealReference) {
  ...
  catalog.setAdvertisedTopics(
      dataset_id_, {PJ::AdvertisedTopic{QStringLiteral("/speed"), PJ::sdk::BuiltinObjectType::kNone}});
  ASSERT_TRUE(referencesTopic(tracker, dataset_id_, QStringLiteral("/speed")));
  const QString key = addScalarTopic(*app_session_, dataset_id_, "/speed");
  ASSERT_FALSE(key.isEmpty());
  ASSERT_FALSE(referencesTopic(tracker, dataset_id_, QStringLiteral("/speed")));
  ...
  tracker.addReference(dataset_id_, QStringLiteral("/speed"));
...
TEST_F(TopicDemandControllerTest, RealHoldArrivingDuringPreviewCancelsTheDisown) {
  ...
  tracker.addReference(dataset_id_, QStringLiteral("/other"));
  tracker.addReference(dataset_id_, QStringLiteral("/speed"));
...
TEST_F(TopicDemandControllerTest, ReallySubscribedDataIsNeverDisowned) {
  ...
  tracker.addReference(dataset_id_, QStringLiteral("/speed"));
  EXPECT_TRUE(catalog.scalarValueAt(key, display_of(200)).has_value());
  tracker.removeReference(dataset_id_, QStringLiteral("/speed"));
  tracker.addReference(dataset_id_, QStringLiteral("/speed"));
  EXPECT_TRUE(catalog.scalarValueAt(key, display_of(200)).has_value());
}
...
TEST_F(TopicDemandControllerTest, PreviewReleasesOnTimeoutWithoutPromotion) {
  ...
  catalog.setAdvertisedTopics(
      dataset_id_, {PJ::AdvertisedTopic{QStringLiteral("/quiet"), PJ::sdk::BuiltinObjectType::kNone}});
  EXPECT_TRUE(referencesTopic(tracker, dataset_id_, QStringLiteral("/quiet")));

  spinFor(200);  // past the injected timeout — nothing ever promoted it
  EXPECT_FALSE(referencesTopic(tracker, dataset_id_, QStringLiteral("/quiet")));
}
...
TEST_F(TopicDemandControllerTest, CensusRunsOncePerTopicButManualPreviewBypassesMemory) {
  ...
  controller_->requestFieldPreview(dataset_id_, QStringLiteral("/quiet"));
  EXPECT_TRUE(referencesTopic(tracker, dataset_id_, QStringLiteral("/quiet")));
}
...
TEST_F(TopicDemandControllerTest, RequestFieldPreviewNoOpsWhenTopicAlreadyReferenced) {
  ...
  // touch it.
  controller_->requestFieldPreview(dataset_id_, QStringLiteral("/speed"));
  EXPECT_TRUE(referencesTopic(tracker, dataset_id_, QStringLiteral("/speed")));
...
TEST_F(TopicDemandControllerTest, CensusCapsConcurrentPreviewsThenRunsTheRest) {
  ...
  for (int i = 0; i < 12; ++i) {
    topics.push_back(PJ::AdvertisedTopic{QStringLiteral("/scalar_%1").arg(i), PJ::sdk::BuiltinObjectType::kNone});
  }
