...
// Standalone visual demo for PJ::CheckButton (pill toggle) + PJ::ColorPickerWidget
// (swatch). Shows both CheckButton states and the "Override color" row layout
...
int main(int argc, char** argv) {
  ...
  root->resize(360, 260);
  root->setWindowTitle(QStringLiteral("pj_widgets — input height check"));
  root->show();

  // Print the resolved heights once laid out so the QSS can be tuned by number.
  auto report = [=]() {
    qInfo(
        "HEIGHTS  LineEdit=%d  Combo=%d  DoubleScrubber=%d  IntScrubber=%d  CheckButton=%d  Swatch=%d",
        line_edit->height(), combo->height(), dscrub->height(), iscrub->height(), off->height(), swatch->height());
  };

  if (!screenshot.isEmpty()) {
    QTimer::singleShot(300, root, [root, screenshot, &app, report] {
      report();
      const QPixmap pix = root->grab();
