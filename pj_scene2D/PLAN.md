# pj_scene2D Implementation Plan
...
| M8 + M15 | `0dd0160` | MCAP→ObjectStore integration (4 tests) + pj_scene2d_widgets with QRhiWidget (MediaViewerWidget: GPU rendering, zoom/pan, pre-compiled shaders) + demo binary. M15 merged into M8 |
...
| CodecPipeline | `10f86b3` | CodecPipeline + codecs + split pj_scene2d_widgets libs |
...
**Scope decision — pj_scene2D renders image-space only.**

`REQUIREMENTS.md §4.1` currently lists **2D `ScenePrimitive` (MarkersData
with `z = 0`)** as a pj_scene2D render target. This is wrong for the
three-widget-family architecture. World-space markers carry `pose`,
`frame_id`, and `Point3`, and to render them on a pj_scene2D image viewer
we would need:
...
`docs/APP_IMPLEMENTATION_PLAN.md §2.4`. Duplicating it inside pj_scene2D
...
families design is meant to avoid. It also matches how pj_scene2D already
...
table) to remove 2D `ScenePrimitive` from pj_scene2D's render contract.
pj_scene2D renders `Image`, `VideoFrame`, and `ImageAnnotation`. That is
the full list.
...
3. **No `pj_scene2d_widgets` wrapper yet.** Roughly one week's work
   once `pj_runtime` scaffolding exists; not blocking today.
...
| `SceneDecoder` (canonical Protobuf → typed primitives) | ARCH §4.3 | ✓ single decoder for `foxglove.ImageAnnotations` (full coverage: points, circles, texts, all colour fields). Source-format conversion (CDR `vision_msgs`/`yolo_msgs`, …) lives loader-side in `pj_scene2D/demos/cdr_*_to_image_annotation.{h,cpp}` and writes canonical bytes via `pj_scene_protocol::serializeImageAnnotation` before push. |
...
2. **ImageAnnotation** — vector overlays in pixel-space (bounding
   ...
   rasterizer. Self-contained to pj_scene2D.
3. **World-space markers** — not pj_scene2D. Moves to `pj_3d_widgets`
...
- ✓ Update `REQUIREMENTS.md §4.1`, `datatypes_2D.md §4`,
  `ARCHITECTURE.md §8.1` to drop 2D `ScenePrimitive` from pj_scene2D
...
- Implement `MediaIndexRegistry`
  (`pj_scene2d_core/media_index_registry.{h,cpp}`) plus the C ABI slot
...
- ✓ Source-format adapters (CDR `vision_msgs/msg/Detection2DArray`,
  ...
  `pj_scene2D/demos/cdr_*_to_image_annotation.{h,cpp}` plus
  ...
  formats (CSV, RLDS, custom) add new adapter files; pj_scene2D doesn't
  change.
...
- `pj_scene2d_widgets` `IDataWidget` wrapper — gated on
  `pj_runtime` Phase 0 scaffolding. Roughly one week when that
  lands.

#### Recent removals (for the record)

- **MpvBackend / VideoViewerWidget / pj_scene2d_widgets_video** — removed
...
- `pj_scene2d_widgets` requires Qt 6.8+ (gracefully skips if not found).
...
## Phase 2 — pj_scene2D core (images from file)
...
### M6: pj_scene2d_core foundation types

**Scope**: `FrameSlot`, `CancelToken`, `DecodedFrame`, `CompositeFrame`
types. Pure C++, no Qt, no external deps beyond pj_base.

**Files**:
- `pj_scene2D/core/include/pj_scene2d_core/frame_slot.h`
- `pj_scene2D/core/include/pj_scene2d_core/cancel_token.h`
- `pj_scene2D/core/include/pj_scene2d_core/decoded_frame.h`
- `pj_scene2D/core/CMakeLists.txt`
- `pj_scene2D/tests/frame_slot_test.cpp`
- `pj_scene2D/tests/cancel_token_test.cpp`
...
**Files**:
- `pj_scene2D/core/include/pj_scene2d_core/image_decoder.h`
- `pj_scene2D/core/src/image_decoder.cpp`
- `pj_scene2D/tests/image_decoder_test.cpp`
- `pj_scene2D/core/CMakeLists.txt` (link turbojpeg)
...
**Files**:
- `pj_scene2D/core/include/pj_scene2d_core/media_source.h`
- `pj_scene2D/core/include/pj_scene2d_core/image_pipeline_source.h`
- `pj_scene2D/core/src/image_pipeline_source.cpp`
- `pj_scene2D/widgets/include/pj_scene2d_widgets/media_viewer_widget.h`
- `pj_scene2D/widgets/src/media_viewer_widget.cpp`
- `pj_scene2D/widgets/CMakeLists.txt`
- `pj_scene2D/demos/mcap_image_viewer.cpp` (demo binary)
- `pj_scene2D/tests/decoded_frame_test.cpp`
...
**Scope**: Define a `VideoBackend` interface that pj_scene2d_widgets uses.
...
**Files**:
- `pj_scene2D/core/include/pj_scene2d_core/video_backend.h`
  (abstract interface: open, seek, pause, stepForward, stepBackward,
  duration, position; frame delivery via OpenGL FBO or callback)
- `pj_scene2D/widgets/include/pj_scene2d_widgets/mpv_backend.h`
- `pj_scene2D/widgets/src/mpv_backend.cpp`
- `pj_scene2D/widgets/CMakeLists.txt` (link libmpv via pkg-config)
...
**Files**:
- `pj_scene2D/widgets/include/pj_scene2d_widgets/video_viewer_widget.h`
- `pj_scene2D/widgets/src/video_viewer_widget.cpp`
- `pj_scene2D/demos/mp4_video_viewer.cpp`
...
**Files**:
- `pj_scene2D/demos/mcap_multi_viewer.cpp`
...
**Files**:
- `pj_scene2D/core/include/pj_scene2d_core/compositor.h`
- `pj_scene2D/core/src/compositor.cpp`
- `pj_scene2D/tests/compositor_test.cpp`
...
**Files**:
- `pj_scene2D/demos/simulated_stream.cpp`
- `pj_scene2D/tests/streaming_test.cpp`
...
**Files**:
- `pj_scene2D/demos/live_viewer.cpp`
...
**Files**:
- `pj_scene2D/widgets/src/media_viewer_widget.cpp` (rewrite)
- `pj_scene2D/widgets/shaders/yuv_to_rgb.vert`
- `pj_scene2D/widgets/shaders/yuv_to_rgb.frag`
...
│   ├── include/pj_scene2d_core/
│   └── src/
├── widgets/
│   ├── CMakeLists.txt          # static lib, Qt 6.8+
│   ├── include/pj_scene2d_widgets/
│   ├── src/
│   └── shaders/
├── tests/
├── demos/
...
|------------|--------|---------|
| turbojpeg | system pkg or conan | pj_scene2d_core (ImageDecoder, ThumbnailCache) |
| ~~libmpv~~ | ~~system pkg~~ | ~~removed~~ |
| FFmpeg (libavcodec, libavformat, libavutil, libswscale) | system pkg | pj_scene2d_core (FfmpegDecoder, FfmpegBackend) |

### .gitignore additions

```
pj_scene2D/testdata/potato.mcap
```
