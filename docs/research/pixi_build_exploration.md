...
| ffmpeg/8.1 (custom LGPL build) | `ffmpeg 8.1.2` **`build = "lgpl_*"`** | must pin the build string — default solve picks the `gpl_*` variant |
...
## The 4 gaps and their recipes (`recipes/`)

None of these exist on conda-forge. Each recipe replicates the exact imported
targets the Conan packages provided (verified against the Conan-generated
configs in the main checkout's `build/`):
...
Each recipe ships a committed, platform-neutral `*Config.cmake` (using
...
depends on these 4 packages, and any `pixi run`/`pixi install` solves the
...
- **FFmpeg feature surface.** Conan built a minimal LGPL decoder set (h264,
  hevc, mjpeg, av1/dav1d; file protocol only; no encoders/muxers). conda-forge
  `lgpl_*` ffmpeg is LGPL-license-clean but *feature-complete* (all LGPL
  decoders, encoders, network protocols, ~12 MB + deps). Same license
  posture, bigger attack/bloat surface. Fine for dev; for release packaging
  we'd keep the current AppImage pipeline or accept the size.
...
# 1. Build the 4 gap packages (once; re-run only when a recipe changes):
pixi exec --spec "rattler-build>=0.66" \
  rattler-build build --recipe-dir recipes/ -m recipes/variants.yaml \
    -c conda-forge --output-dir local-channel
